...
const Profile = () => {
    ...
    try {
      if (selectedAddressId) {
        ...
        const response = await axios.put(
          `${SERVER_URL}/user/editAddress/${selectedAddressId}`,
          {
            ...addressData,
            userId,
          },
  ...
  const fetchAddresses = async () => {
    try {
      const response = await axios.get(
        `${SERVER_URL}/user/addresses/${userId}`
      );
      if (response.status === 200) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data.profileImage) {
          setProfileImage(response.data.profileImage); // Set the profile image if available
        }
      } else {
  ...
  const handleAddressDelete = async (addressId) => {
    try {
      const response = await axios.delete(
        `${SERVER_URL}/user/deleteAddress/${addressId}`,
        { data: { userId } }
      );
      if (response.status === 200) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${SERVER_URL}/user/uploadProfileImageUser/${userId}`,
        payload,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
          ...
          <div className="flex items-center mb-6 relative">
            <div className="relative">
              ...
              <button
                ...
                className="absolute bottom-0 right-0 bg-blue-700 text-white rounded-full p-1 hover:bg-blue-600 transition-colors"
              >
          ...
          <div className="space-y-4">
            <button
              className={`flex items-center space-x-3 w-full p-2 rounded-lg hover:bg-blue-100 ${
                activeTab === "personalInfo" && "bg-blue-100"
              }`}
            ...
            <button
              className={`flex items-center space-x-3 w-full p-2 rounded-lg hover:bg-blue-100 ${
                activeTab === "manageAddress" && "bg-blue-100"
              }`}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <h3 className="text-xl font-semibold text-gray-800">
                        {address.userName} - {address.addressLine}
                      </h3>
                      <button
                        onClick={() => editAddress(address)}
                        className="text-blue-600 hover:bg-blue-100 rounded-full p-2 transition duration-200"
